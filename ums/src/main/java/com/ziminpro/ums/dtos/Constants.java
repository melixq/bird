...
public class Constants {
    ...
    public static final String GET_USER_BY_ID_FULL = "SELECT * FROM " + TABLE_USERS + " LEFT JOIN " + TABLE_USERS_ROLES
            ...
            + ".`id`=UUID_TO_BIN(?);";
    public static final String GET_USER_BY_GITHUB_ID = "SELECT * FROM " + TABLE_USERS + " LEFT JOIN " + TABLE_USERS_ROLES
            + " ON " + TABLE_USERS_ROLES + ".`users_id` = " + TABLE_USERS + ".`id` LEFT JOIN " + TABLE_ROLES + " ON "
            + TABLE_USERS_ROLES + ".`roles_id` = " + TABLE_ROLES + ".`id` LEFT JOIN " + TABLE_LAST_VISIT + " ON "
            + TABLE_USERS + ".`last_visit_id` = " + TABLE_LAST_VISIT + ".`id` WHERE " + TABLE_USERS
            + ".`github_id`=?;";
    public static final String GET_USER_BY_EMAIL = "SELECT * FROM " + TABLE_USERS + " LEFT JOIN " + TABLE_USERS_ROLES
            + " ON " + TABLE_USERS_ROLES + ".`users_id` = " + TABLE_USERS + ".`id` LEFT JOIN " + TABLE_ROLES + " ON "
            + TABLE_USERS_ROLES + ".`roles_id` = " + TABLE_ROLES + ".`id` LEFT JOIN " + TABLE_LAST_VISIT + " ON "
            + TABLE_USERS + ".`last_visit_id` = " + TABLE_LAST_VISIT + ".`id` WHERE " + TABLE_USERS
            + ".`email`=?;";
    ...
    public static final String CREATE_USER_WITH_GITHUB = "INSERT INTO " + TABLE_USERS
            + " (`id`, `name`, `email`, `github_id`, `avatar_url`, `password`, `created`, `last_visit_id`, `token_version`) VALUES "
            + "(UUID_TO_BIN(?), ?, ?, ?, ?, NULL, ?, NULL, ?);";
    public static final String UPDATE_USER_BY_GITHUB_ID = "UPDATE " + TABLE_USERS
            + " SET `name`=?, `email`=?, `avatar_url`=? WHERE `github_id`=?;";
